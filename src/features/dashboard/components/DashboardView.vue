...
async function charger() {
  ...
  try {
    commandes.value = await fetchCommandesMagasin(session.magasinId)
  } catch {
...
useWebSocketTopic<CommandeResponse>(`/topic/magasins/${session.magasinId}/commandes`, (nouvelleCommande) => {
...
async function handleValiderRecuperation() {
  ...
  try {
    const commande = await validerRecuperation(session.magasinId, codeSaisi.value.trim().toUpperCase())
